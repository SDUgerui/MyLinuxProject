...
PthreadPool::~PthreadPool()
{
    ...
    pthread_mutex_destroy(&lock); // 上锁的时候可以销毁锁
    pthread_cond_destroy(&notify);

    delete[] threads;   //销毁线程
}

int PthreadPool::Init(unsigned int num)   //初始并创造池子
{
    // 初始化互斥锁和条件变量
    do
    {
        if (num <= 0)
            break;
        if (pthread_mutex_init(&lock, NULL))  //初始化锁
            break;
        if (pthread_cond_init(&notify, NULL))  //初始化条件变量 
            break;

        // 初始化线程数组
        threads = new pthread_t[num];   //在堆中开辟线程标识数组，动态可分配的
...
// 线程运行函数
void *PthreadPool::threadpool_thread(void *threadpool)   //创建的每个线程函数都执行，输入的池，提取其中的任务列表
{
    PthreadPool *pool = (PthreadPool *)threadpool; // 获取当前实例
    while (1)   //所有线程一直在循环，不会被回收
    {
        /* 取得互斥锁资源 */
        pthread_mutex_lock(&(pool->lock));
        while ((pool->thread_queue.empty()) && (!pool->shutdown))
        {
            /* 任务队列为空，且线程池没有关闭时阻塞在这里，重要！如果有任务传入，则自动接管人物，不需要后期再分配*/
        ...
        // 开始运行任务,运行任务函数
        (*(Runable.function))(Runable.argument);   //取函数指针操作！
        // 结束，回到等待
    }
    // 更新正在运行的线程数
    pool->running_num--;
